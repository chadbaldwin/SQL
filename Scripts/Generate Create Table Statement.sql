		...
		FROM sys.dm_exec_describe_first_result_set(@sql, NULL, 1) x
			...
			CROSS APPLY (
				SELECT ColName = IIF(x.[name] IN ( -- Add sqare brackets only to reserved keywords or names containing unescaped characters
						'action','address','affinity','algorithm','allow_encrypted_value_modifications','allow_page_locks','allow_row_locks','ansi_defaults','ansi_null_dflt_on','ansi_nulls','ansi_padding','ansi_warnings','app_name','application','arithabort','automated_backup_preference','availability_mode','backup_priority','backup_storage_redundancy','batchsize','broker_instance','bucket_count','catalog','cert','checksum','classification','clear_port','cluster_type','columns','committed','compatibility_level','compression_delay','concat_null_yields_null','context_info','contract','conversation_group_id','conversation_handle','count','counter','data','data_compression','data_source','database','database_name','database_principal_id','date','date_format','datefirst','dateformat','day','dayofyear','days','db_failover','db_id','db_name','dbid','dbidexec','deadlock_priority','default_database','default_logon_domain','definition','delayed_durability','deny','description','desired_state','digest','dtc_support','durability','enabled','encoding','encrypted','encrypted_value','encryption_type','endpoint','endpoint_url','entity_id','entity_name','error_message','error_number','error_severity','error_state','event','expiredate','expiry_date','failover_mode','failure_condition_level','federated_service_account','fetch_status','field_terminator','file_id','file_name','filegroup_id','filename','filter','first','first_row','flush_interval_seconds','format_type','guid','hash','header_limit','health_check_timeout','high','host_name','identity','ignore_dup_key','index','information_type','information_type_id','initiator','input_options','interval_length_minutes','job_tracker_location','key_guid','key_id','key_name','key_path','key_store_provider_name','label','label_id','langid','language','length','level','lifetime','location','lock_escalation','lock_timeout','lockres','login_type','low','max_duration','max_outstanding_io_per_volume','max_plans_per_query','max_storage_size_mb','message','messages','minutes','mirror_address','month','monthname','name','namespace','object_id','object_name','objid','open','optimize_for_sequential_key','option','or','order','output_options','parameters','partition_id','password','path','permission_name','permission_set','permissions','platform','population','port','precision','predicate','priority','procedure_name','program_name','provider','quarter','query_capture_mode','quoted_identifier','rank','read','read_only_routing_url','recovery','reject_sample_value','reject_type','reject_value','rejected_row_location','remote_service_name','replace','required_synchronized_secondaries_to_commit','resource_manager_location','restart','retention','retention_period','role','root','row','row_terminator','rowcount','rows','schema_id','schema_name','scope_id','secondary_type','seeding_mode','sensitivity','sequence','serde_method','server','service_name','session_context','session_id','session_timeout','setopts','shard_map_name','sid','signature','site','size_based_cleanup_mode','source','sql','sql_handle','ssl_port','start_date','state','statement','status','string_delimiter','subject','system','target','text','textptr','timeout','timestamp','trim','type','type_desc','type_name','uid','url','use_type_default','user','user_id','user_name','validation','value','version','weekday','weight','year'
