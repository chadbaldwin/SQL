...
SELECT i.[object_id], i.index_id
	...
	, x.KeyCols, x.InclCols, x.KeyBitmap1, x.KeyBitmap2, x.InclBitmap1, x.InclBitmap2
INTO #tmp_idx
FROM sys.indexes i
	...
	CROSS APPLY (
		SELECT KeyCols     = STRING_AGG(IIF(ic.is_included_column = 0, CONCAT(IIF(ic.is_descending_key = 1, '-', ''), ic.column_id), NULL), ',') WITHIN GROUP (ORDER BY ic.key_ordinal)+','
			...
			,  KeyBitmap1  = CONVERT(bigint, SUM(IIF(ic.is_included_column = 0 AND ic.column_id >  0 AND ic.column_id <=  62, POWER(@2, ic.column_id     ), 0)))
...
--	, x.KeyBitmap1, x.KeyBitmap2, x.InclBitmap1, x.InclBitmap2
	, N'█' [██], y.IndexName, y.KeyCols, y.InclCols
